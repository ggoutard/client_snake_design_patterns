...
public class Snake extends GameElement {
	...
	public boolean isDead(InputMap map){
		...
		if (!this.dead && !this.isInvisible()){
			...
			int id_ennemi = 0;
			if(this.id == 0) id_ennemi = 1;
			
			//ArrayList<Position> positions_ennemi = map.getStart_snakes().get(id_ennemi).getPositions();
	...
	{
		if (SnakeGame.isLegalMove(this, movement)) this.action = movement ;
	}
}
