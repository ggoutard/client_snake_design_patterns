...
public class PanelSnakeGame extends JPanel{
		...
		for(int x=0; x<sizeX; x++)
		{
			...
			for(int y=0; y<sizeY; y++)
			{
				if (walls[x][y]){
					...
					try {
						Image img = ImageIO.read(new File("src/tp1progreseau/images/wall.png"));
		...
		for(int i = 0; i < positions.size(); i++) {
			...
			try {
				if(featuresSnake.getColorSnake() == ColorSnake.Green) {
					img = ImageIO.read(new File("src/tp1progreseau/images/snake_green_" + cpt_img + ".png"));
				} else if(featuresSnake.getColorSnake() == ColorSnake.Red ) {
					img = ImageIO.read(new File("src/tp1progreseau/images/snake_red_" + cpt_img + ".png"));	
				}
	...
	void paint_Item(Graphics g, FeaturesItem featuresItem){
		...
		if (featuresItem.getItemType() == ItemType.APPLE) {
			try {
				Image img = ImageIO.read(new File("src/tp1progreseau/images/apple.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (featuresItem.getItemType() == ItemType.BOX) {
			try {
				Image img = ImageIO.read(new File("src/tp1progreseau/images/mysteryBox.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (featuresItem.getItemType() == ItemType.SICK_BALL) {
			try {
				Image img = ImageIO.read(new File("src/tp1progreseau/images/sickBall.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (featuresItem.getItemType() == ItemType.INVINCIBILITY_BALL) {
			try {
				Image img = ImageIO.read(new File("src/tp1progreseau/images/invicibleBall.png"));
				g.drawImage(img, (int)pos_x, (int)pos_y, (int)stepx, (int)stepy, this);
			} catch (IOException e) {
				e.printStackTrace();
			}
