    ...
    private static class ThreadInput implements Runnable, Observer {
        ...
        public void update(Observable o, Object arg) {
        	...
        	if (arg instanceof Message) 
            {
                ...
                System.out.print("Lancement de la recherche d'aversaire...\nSur la carte : " + message.getMessage().replace("\"src/tp1progreseau/layouts/", "") );
        ...
        public void openViewGame() {
            if (this.accueil != null) {
                new Thread(() -> {
                    try {
                        this.accueil.ChangeStateChargement("Lancement de la partie dans 3...");
                        Thread.sleep(1000); 
                        
                        this.accueil.ChangeStateChargement("Lancement de la partie dans 2...");
                        Thread.sleep(1000); 
                        this.accueil.ChangeStateChargement("Lancement de la partie dans 1...");
                        Thread.sleep(1000); 
                        this.accueil.dispose();
                        this.accueil = null;
                    } catch (InterruptedException e) {
                        System.out.println("Erreur dans le timer : " + e.getMessage());
                    }
                }).start();
            }
        }
